...
def make_final_video(reddit_details):
    output_files = {}
    
    for reddit in reddit_details:
        ...
        if background_clip_duration < total_overlay_duration:
            # Calculate the number of times the background clip needs to be repeated
            repetitions = int(total_overlay_duration / background_clip_duration)

            # Calculate the remaining duration after repetitions
            remaining_duration = total_overlay_duration % background_clip_duration

            # Create a loop to concatenate multiple instances of the background video
            background_clips = [background_clip] * repetitions

            # Add a partial background clip if there's remaining duration
